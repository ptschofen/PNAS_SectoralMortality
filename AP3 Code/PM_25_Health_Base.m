...
%% For Cause-Specific Damages

% All Cause = 1 = All_Mort{1,1}
% CP = 2 = All_Mort{2,1}
% CV = 3 = All_Mort{3,1}
% IHD = 4 = All_Mort{4,1}
...
Cause = 1;
%%
% Deaths = ((Mort2002.*((One'*((exp(0.006.*PM_25_B')))-1)'))).*Pop_over_30;

% load C:\APEEP_V_2005\Model\BenMAP_Mort_MAT;
% load C:\APEEP_V_2005\Model\Mort_2005_ZSU;

%% Log-linear Model
% Deaths = ((Mortality{3,1}.*((One'*((exp(0.006.*PM_25_B')))-1)'))).*Pop_over_30;

%% BenMAP Form
Deaths = (Mortality{3,1}.*(One'*(1-(1./(exp(0.005826891.*PM_25_B')))))').*Pop_over_30;
% Deaths = (Mortality{3,1}.*(One'*(1-(1./(exp(0.01.*PM_25_B')))))').*Pop_over_30;
...
% If using spatially-uniform rates: BenMAP_Mort
% If using zero replacement rates: Mort_2005_ZSU
DB = sum(sum(Deaths));

   %Cause-Specific Deaths
   
% Deaths_CP = (Mortality{9,1}.*(One'*(1-(1./(exp(0.0153.*PM_25_B')))))').*Pop_over_30;
% Deaths_CV = (Mortality{10,1}.*(One'*(1-(1./(exp(0.0206.*PM_25_B')))))').*Pop_over_30;
% Deaths_IHD = (Mortality{11,1}.*(One'*(1-(1./(exp(0.0306.*PM_25_B')))))').*Pop_over_30;
% 
...
%% Log-linear Model
% Infant = ((Mort2002.*((One'*((exp(0.007.*PM_25_B')))-1)'))).*Pop_Infant;

%% BenMAP Form
Infant = (Mortality{3,1}.*(One'*(1-(1./(exp(0.006765865.*PM_25_B')))))').*Pop_Infant;
...
% Deaths = ((Mortality{3,1}.*((One'*((exp(0.0008.*PM_25_B')))-1)').*Pop_over_30));
...
%% Abbey, et al. (1995)
% Cases_CB = 1-(1./((1-0.00378)*One'*(exp(0.013185041*PM_25_B')))+0.00378)'.*Pop_over_30.*0.00378.*(1-0.0476);
% Cases_CB = ((0.00378.*One'*(1-(1./(exp(0.09132.*PM_25_B')))))').*CB_Pop;
% CB_PM25_B = sum(sum(Cases_CB.*WTP_Morb));
% Spatial_Morb_B = Cases_CB.*WTP_Morb;
